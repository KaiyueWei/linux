...
static void shrink_node(pg_data_t *pgdat, struct scan_control *sc)
{
	...
	/*
	 * cache_ext: let BPF policy propose eviction candidates before
	 * the kernel's own LRU/MGLRU reclaim runs. The BPF program fills
	 * (inode_id, page_index) integer tuples; the kernel resolves them
	 * to struct folio * via xarray lookup — no raw kernel pointers
	 * pass through BPF arena memory.
	 */
	if (cache_ext_enabled()) {
